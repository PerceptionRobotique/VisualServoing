...
./PGMPerspVS_E2H_levels ../PGM_Perspective_VisualServoing_E2H_media/calibration/calib_640_512.xml 6 5.0 1 ../PGM_Perspective_VisualServoing_E2H_media/images_full/Noodles_mask.png 1000 350 0 0 70
...
*/
  ...
  const char *filenameMask = NULL;
  if(argc > 5)
    filenameMask = argv[5];

  try
  {
    vpImage<unsigned char> IMask_load;

		vpImageIo::read(IMask_load,filenameMask) ;

    vpImageTools::resize(IMask_load, IMask, larg, haut,  vpImageTools::vpImageInterpolationType::INTERPOLATION_CUBIC);//INTERPOLATION_LINEAR);//
	}
	catch(...)
  {
    std::cout << "mask unreadable" << std::endl;
	  IMask.resize(haut,larg);
    IMask = 255;
  }

#ifdef WITHROBOT       
	C_UR UR10("192.168.1.3", 30002, 0.14); //35);//40);//

  vpHomogeneousMatrix bMc, bMt;

//  bMc.buildFrom(0.1051163875,  -0.8934777804,  0.2317511933,  -1.487098181,  -0.5337782593,  0.5142973009); // from EyeToHand_Calibration (-rpy)
//  bMc.buildFrom(0.0996250801,  -0.8913291788,  0.2300101574,  -1.486553165,  -0.5198658721,  0.5245972798); // from EyeToHand_Calibration (-rpy)
//  bMc.buildFrom(0.2115828142,  -0.8956598641,  0.2403836407,  -1.438052,  -0.6848745644,  0.68328943); // from EyeToHand_Calibration (-rpy) 29/10/2020
  bMc.buildFrom(0.2090042972,  -0.8983311576,  0.2506011417,  -1.454643113,  -0.6926131865,  0.6488090123); // from EyeToHand_Calibration (-rpy) 05/11/2020

  vpVelocityTwistMatrix cVt, cVb, bVt;

  cVb.buildFrom(bMc.inverse()); // from EyeToHand_Calibration
...
/*
  //Ayataka
	j_init[0] = vpMath::rad(-87.76);
	j_init[1] = vpMath::rad(-81.54);
	j_init[2] = vpMath::rad(-104.27);
	j_init[3] = vpMath::rad(-84.14);
	j_init[4] = vpMath::rad(89.75);
	j_init[5] = vpMath::rad(-11.95);
*/
/*  
  //Kitsune
	j_init[0] = vpMath::rad(-87.82);
	j_init[1] = vpMath::rad(-81.91);
	j_init[2] = vpMath::rad(-104.08);
	j_init[3] = vpMath::rad(-83.38);
	j_init[4] = vpMath::rad(90.53);
	j_init[5] = vpMath::rad(-138.88);
*/
/*
  //Noodles
	j_init[0] = vpMath::rad(-87.76);
	j_init[1] = vpMath::rad(-81.54);
	j_init[2] = vpMath::rad(-104.27);
	j_init[3] = vpMath::rad(-84.13);
	j_init[4] = vpMath::rad(89.75);
	j_init[5] = vpMath::rad(-92.22);
*/
/*
  //Plastic
	j_init[0] = vpMath::rad(-87.76);
	...
	j_init[2] = vpMath::rad(-104.27);
	j_init[3] = vpMath::rad(-84.14);
	j_init[4] = vpMath::rad(89.76);
	j_init[5] = vpMath::rad(8.06);
/*
  /*
  //Cheese
	j_init[0] = vpMath::rad(-87.76);
...
*/
  //Curry
	...
	j_init[3] = vpMath::rad(-84.13);
	j_init[4] = vpMath::rad(89.76);
	j_init[5] = vpMath::rad(-0.97);
/*
  //Corn
	j_init[0] = vpMath::rad(-87.76);
	j_init[1] = vpMath::rad(-81.54);
	j_init[2] = vpMath::rad(-104.26);
	j_init[3] = vpMath::rad(-84.15);
	j_init[4] = vpMath::rad(89.76);
	j_init[5] = vpMath::rad(-96.66);
*/
/*
  //Suntory
	j_init[0] = vpMath::rad(-87.76);
	...
	j_init[3] = vpMath::rad(-84.15);
	j_init[4] = vpMath::rad(89.76);
	j_init[5] = vpMath::rad(-62.00);
*/
/*
  //Cheese
	j_init[0] = vpMath::rad(-87.82);
	j_init[1] = vpMath::rad(-81.91);
	j_init[2] = vpMath::rad(-104.09);
	j_init[3] = vpMath::rad(-83.37);
	j_init[4] = vpMath::rad(90.54);
	j_init[5] = vpMath::rad(-85.61);
*/
/*
  //Chips
	j_init[0] = vpMath::rad(-87.77);
	j_init[1] = vpMath::rad(-81.954);
	j_init[2] = vpMath::rad(-104.27);
	j_init[3] = vpMath::rad(-84.15);
	j_init[4] = vpMath::rad(89.77);
	j_init[5] = vpMath::rad(-85.41);
*/
/*
  //Kitsune
	j_init[0] = vpMath::rad(-87.76);
...
*/
  ...
  ficDesiredPose.close();
    ...
    bool dofs[6] = {true, false, true, false, true, false};
    ...
    //IP_des.setInterpType(IMAGEPLANE_NEARESTNEIGH);
    std::cout << "build" << std::endl;
    IP_des.buildFrom(I_des, perspCam, &IMask); 
...
//    IP_des.toAbsZN(); //prepare pixels intensities ? 
  ...
  if(argc < 8)
  {
    ...
    float gain = 2.0f; bool shallow = false;   //Corn, Gum
    //float gain = 5.0f; bool shallow = false;   //Noodles
    //float gain = 1.0f; bool shallow = false;   //Ayataka, Suntory, Chips
    //float gain = 5.0f; bool shallow = false;   //Kitsune
  ...
  if(rotZ > 180)
  {
    p_init[5] = 179.0*M_PI/180.0;

    std::cout << "Deplacement vers pose intermediaire " << p_init.t() << std::endl;

    UR10.setToolRelativePose(p_init);
  ...
  vpImageTools::resize(Iacq, I_cur, larg, haut, vpImageTools::vpImageInterpolationType::INTERPOLATION_CUBIC);//INTERPOLATION_LINEAR);//
    ...
    IP_cur.buildFrom(I_cur, perspCam, &IMask); 
...
//  vpColVector p;
...
#endif //INDICATORS
  double tms, duree, timeToGetImage;
  ...
  double fact_gain = 1.0;
    ...
    //seuil pour 6DoF 5e-4
    if(((iter < 60) || ((residual_diff > (gain*(5e-4)*residual_back)) )) && (!shallow || (sqrt(v.sumSquare()) > 1e-4)))
...
#ifdef WITHCAMERA
    timeToGetImage = vpTime::measureTimeMs();
		grabber.getFrame(Iacq);
    timeToGetImage = vpTime::measureTimeMs() - timeToGetImage;
    vpImageTools::resize(Iacq, I_cur, larg, haut, vpImageTools::vpImageInterpolationType::INTERPOLATION_CUBIC);//INTERPOLATION_LINEAR);//
#endif
    ...
    //For eye-to-hand control
    UR10.getToolPose(bMt);
    //bMt[0][3] = bMt[1][3] = bMt[2][3] = 0.0;
    bVt.buildFrom(bMt);
    cVt = cVb*bVt;
    ...
    else
    {
      ...
      for(unsigned int i = 0 ; i < 6 ; i++)
      {
        if(dofs[i])
        {
          //fill in line j of cVt_dof
          unsigned int jj = 0;
          for(unsigned int ii = 0 ; ii < 6 ; ii++)
            //if(activated_DoF[ii])
            {
              cVt_dof[j][jj] = cVt[i][ii];
              jj++;
            }
          j++;
        }
    ...
    v_t = -(cVt_dof*tJt).pseudoInverseEigen3()*v;
    
      //saturation to avoid bad robot behavior
      if(v_t[0] < -0.4)
        v_t[0] = -0.4;
      else if(v_t[0] > 0.4)
        v_t[0] = 0.4;
...
#ifdef WITHROBOT
    if(servo_actif)
      UR10.setBaseToolVelocity(v6);
#endif

    duree = vpTime::measureTimeMs() - tms - timeToGetImage;
  ...
  UR10.setCameraVelocity(v6);
    ...
    //save velocity list to file
    s.str("");
    s.setf(std::ios::right, std::ios::adjustfield);
    s << "resultat/velocities.txt";
    filename = s.str();
