...
int C_UR::setToolVelocity(vpColVector &tv)
{
	if(tv.getRows() != 6)
		return -1;
	else
...
/*    vpRotationMatrix bRt(vpThetaUVector(actual_tcp_pose[3], actual_tcp_pose[4], actual_tcp_pose[5]));
    vpTranslationVector btt(actual_tcp_pose[0], actual_tcp_pose[1], actual_tcp_pose[2]);
    btt = -(bRt.t()*btt);
    vpHomogeneousMatrix bMt(btt, bRt);*/

    vpRzyxVector rzyx(vpThetaUVector(actual_tcp_pose[5], actual_tcp_pose[4], actual_tcp_pose[3]));
//    std::cout << rzyx.t() << std::endl;
    vpHomogeneousMatrix tMb;
    UR2Vision(actual_tcp_pose[0], actual_tcp_pose[1], actual_tcp_pose[2], rzyx[0], rzyx[1], rzyx[2], tMb);

//    std::cout << "tMb: " << tMb << std::endl;

    vpHomogeneousMatrix bMt = tMb.inverse();

//    std::cout << vpPoseVector(bMt).t() << std::endl;

    vpVelocityTwistMatrix bVt(bMt);

    vpColVector bv = bVt  * tv;

    osChaine << "speedl([" << bv[0] << ", " << bv[1] << ", " << bv[2] << ", " << bv[3] << ", " << bv[4] << ", " << bv[5] << "], a=0.2, t=0.05)\n";
...
int C_UR::setBaseToolVelocity(vpColVector &bv)
{
	if(bv.getRows() != 6)
		return -1;
	else
...
//    osChaine << "speedl([" << bv[0] << ", " << bv[1] << ", " << bv[2] << ", " << bv[3] << ", " << bv[4] << ", " << bv[5] << "], a=0.1, t=0.05)\n";

//    osChaine << "speedl([" << bv[0] << ", " << bv[1] << ", " << bv[2] << ", " << bv[3] << ", " << bv[4] << ", " << bv[5] << "], a=0.1, t=0.035)\n";
//    osChaine << "speedl([" << bv[0] << ", " << bv[1] << ", " << bv[2] << ", " << bv[3] << ", " << bv[4] << ", " << bv[5] << "], a=0.1, t="<<delta_t<<")\n";
    osChaine << "speedl([" << bv[0] << ", " << bv[1] << ", " << bv[2] << ", " << bv[3] << ", " << bv[4] << ", " << bv[5] << "], a=1.0, t="<<delta_t<<", aRot=1.0)\n";//, t=0.035)\n";
...
int C_UR::setToolRelativePose(vpColVector &p)
{
	if(p.getRows() != 6)
		return -1;
	else
...
//meters and radians
int C_UR::setToolPoseRaw(vpColVector &p)
{
	if(p.getRows() != 6)
		return -1;
	else
    ...
    osChaine << "movel(p[" << p[0] << ", " << p[1] << ", " << p[2] << ", " << p[3] << ", " << p[4] << ", " << p[5] << "], a=0.1, v=0.5)\n";
...
int C_UR::getToolPoseRaw(vpColVector &p)
{
...
int C_UR::getToolPose(vpHomogeneousMatrix &bMt)
{
  std::vector<double> actual_tcp_pose = rtde_control.getActualTCPPose();

//  std::cout << "actual_tcp_pose:"  <<actual_tcp_pose[0] << " " << actual_tcp_pose[1] << " " << actual_tcp_pose[2] <<" " <<actual_tcp_pose[3] << " " << actual_tcp_pose[4] << " " << actual_tcp_pose[5] << std::endl;
...
void C_UR::UR2Vision(double X, double Y, double  Z, double Rx, double Ry, double Rz, vpHomogeneousMatrix &tMb)
{
    //Bloc rotation de tMb
    vpRotationMatrix tRb;
    //Vecteur translation de tMb
    vpTranslationVector ttb;
    
    //Vecteur des angles de rotation d'Euler
    vpRzyxVector rzyx;
    //Positions dans les repères Fb et Ft
    vpColVector bX(3), tX(3);
