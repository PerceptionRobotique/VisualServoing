...
public:
  ...
  int setBaseToolVelocity(vpColVector &bv);
  int setToolVelocity(vpColVector &v);
  int getToolPose(vpHomogeneousMatrix &bMt);
  int setToolRelativePose(vpColVector &p);
  int setToolPoseRaw(vpColVector &p);
  int getToolPoseRaw(vpColVector &p);

  void UR2Vision(double X, double Y, double  Z, double Rx, double Ry, double Rz, vpHomogeneousMatrix &tMb);
