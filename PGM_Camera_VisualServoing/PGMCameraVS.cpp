/*!
 ...
 \brief Photometric Gaussian Mixture (PGM) -based Visual Servoing. SSD compares
current and desired PGMs for perpsective camera control (6 DOFs), exploiting PeR
core, core_extended, io, features, estimation and sensor_pose_estimation modules
 * example command line :
./build/PGMPerspVS
./PGM_Perspective_VisualServoing_media/calibration/calib_640_512.xml 6 5.0 1000
10 -10 \param xmlFic the perspective camera calibration xml file \param redFac
the image scale reduction factor \param lambda_g the initial Gaussian expansion
parameter \param metFac the factor to transform shiftX to meters \param shiftX
the signed lateral shift (in coherent units regarding metFac) \param shiftZ the
signed depth shift (in coherent units regarding metFac) \param rotY the signed
 ...
 *
 * 1. Loading a perspective camera model from an XML file got from the MV
 * calibration software
 * 2. VS objects initialization, considering the pose control of a perspective
 * camera from the feature set of photometric non-normalized Gaussian mixture 2D
 * samples compared thanks to the SSD
 * 3. Successive computation of the current festures set for every acquired
 * image that are used to control the camera toward the desired image
...
int main(int argc, char **argv) {
  ...
  // 1. Loading a perpsective camera from an XML file got from the MV
  // calibration software
  if (argc < 2) {
#ifdef VERBOSE
  ...
  std::cout << "the camera base intrinsic parameters are alpha_u = "
            << _camera->getau() << " ; alpha_v = " << _camera->getav()
            << " ; u_0 = " << _camera->getu0()
            << " ; v_0 = " << _camera->getv0() << std::endl;
  ...
  */
...
#ifdef WITHCAMERA
  vpImage<unsigned char> Iacq, IacqTmp;

  // Parametres intrinseques pour FlirCam
  int larg = 100, haut = 100;

  CamFlirSpinnaker<unsigned char> grabber(larg, haut, 8, 0);

  // Acquisition
  grabber.getFrame(IacqTmp);
  larg = IacqTmp.getWidth() / redFac;
  haut = IacqTmp.getHeight() / redFac;

  vpImageTools::resize(IacqTmp, Iacq, larg, haut,
                       vpImageTools::vpImageInterpolationType::
                           INTERPOLATION_CUBIC); // INTERPOLATION_LINEAR);//
  vpImageTools::resize(Iacq, I_des, larg, haut,
                       vpImageTools::vpImageInterpolationType::
                           INTERPOLATION_CUBIC); // INTERPOLATION_LINEAR);//
  ...
  //pnnGM_feature.getJacobian() internally computes myJacobian *
  parent_feature.getJacobian(), parent_feature calling itself its own parent
  feature getJacobian until there is no more ancester
  */
  // initialisation de l'AV
  // prPhotometricnnGMS<prCartesian2DPointVec>,
  // prPosePerspectiveEstim
  prCameraPoseEstim<prFeaturesSet<prCartesian2DPointVec,
                                  prPhotometricnnGMS<prCartesian2DPointVec>,
                                  prRegularlySampledCPImage>,
                    prSSDCmp<prCartesian2DPointVec,
                             prPhotometricnnGMS<prCartesian2DPointVec>>>
      servo;
  ...
  _camera->setPrincipalPoint(_camera->getu0() / redFac,
                             _camera->getv0() / redFac);
  servo.setSensor(_camera);

  // prepare the desired image
  prRegularlySampledCPImage<unsigned char> IP_des(
      I_des.getHeight(),
      I_des.getWidth()); // the regularly sample planar image to be set from the
  ...
  vpImageTools::resize(Iacq, I_cur, larg, haut,
                       vpImageTools::vpImageInterpolationType::
                           INTERPOLATION_CUBIC); // INTERPOLATION_LINEAR);//
#endif

  // 3. Successive computation of the current features set for every acquired
  // image that are used to control the camera toward the desired image
  ...
  prRegularlySampledCPImage<unsigned char> IP_cur(I_des.getHeight(),
                                                  I_des.getWidth());
  ...
  prFeaturesSet<prCartesian2DPointVec,
      ...
      fSet_cur;
  t0 = vpTime::measureTimeMs();
  fSet_cur.buildFrom(IP_cur, GP, GP_sample, poseJacobianCompute,
                     updateSampler); // Goulot !
  t0 -= vpTime::measureTimeMs();
  ...
  disp_PGM_cur.init(PGM_cur_u, 100 + I_des.getWidth() + 5,
                    100 + PGM_des_u.getHeight() + 30, "PGM_cur");
  ...
  disp_I_diff.init(Idiff, 100,
                   100 + I_des.getHeight() + 30 + I_cur.getHeight() + 30,
                   "I_cur-I_des (for information)");
  ...
  disp_PGM_diff.init(PGMdiff, 100 + I_des.getWidth() + 5,
                     100 + PGM_des_u.getHeight() + 30 + PGM_cur_u.getHeight() +
                         30,
                     "PGM_cur-PGM_des (minimized)");
  ...
  do {
    ...
    fSet_cur.updateMeasurement(IP_cur, GP, GP_sample, poseJacobianCompute,
                               updateSampler);
  ...
  for (unsigned int i = 0; i < v_p.size(); i++) {
    ...
    ficResiduals << std::fixed << std::setw(11) << std::setprecision(6)
                 << v_residuals[i] << std::endl;
    ficTimes << v_tms[i] << std::endl;

    s.str("");
    s.setf(std::ios::right, std::ios::adjustfield);
    s << "resultat/I/I." << std::setw(4) << std::setfill('0') << i << "."
      << FILE_EXT;
    filename = s.str();
    ...
    s.str("");
    s.setf(std::ios::right, std::ios::adjustfield);
    s << "resultat/PGM/PGM." << std::setw(4) << std::setfill('0') << i << "."
      << FILE_EXT;
    filename = s.str();
    ...
    s.str("");
    s.setf(std::ios::right, std::ios::adjustfield);
    s << "resultat/IErr/IErr." << std::setw(4) << std::setfill('0') << i << "."
      << FILE_EXT;
    filename = s.str();
    ...
    s.str("");
    s.setf(std::ios::right, std::ios::adjustfield);
    s << "resultat/PGMErr/PGMErr." << std::setw(4) << std::setfill('0') << i
      << "." << FILE_EXT;
    filename = s.str();
